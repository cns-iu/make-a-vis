...
export class ColorScale {
  ...
  constructor(start: string, end: string, private outOfDomainColor: string, private strokeOffset = 0) {
  ...
  getQualitativeColorFunction(domain: number[], offset = 0): (value: number) => string {
  ...
  getQuantitativeColorFunction(domain: number[], offset = 0): (value: number) => string {
  ...
  getAddends(categoryCount: number): number[] {
    return Array.of(
